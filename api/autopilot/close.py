...
from stockoracle.autopilot import close_autopilot  # noqa: E402


app = FastAPI(title="StockOracle Autopilot Close API")


class AutopilotCloseRequest(BaseModel):
    force: bool = False
    autopilotToken: str | None = None
...
@app.post("/")
@app.post("/api/autopilot/close")
def execute(payload: AutopilotCloseRequest, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    try:
        _validate_autopilot_access(payload.autopilotToken, authorization)
        return close_autopilot(force=payload.force)
