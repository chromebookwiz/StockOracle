...
function parserFriendlyResponse(rankResponse: Record<string, unknown>, payload: Record<string, unknown>) {
  ...
  const predictions = ranking.map((row, index) => ({
    ...
    modelDisagreement: row.model_disagreement,
    ...
    futureReturnBlend: row.future_return_blend,
  ...
  return {
    ...
    summary: {
      ...
      topSetupExpectedEdge: (metrics as Record<string, unknown>).top_setup_expected_edge ?? null,
      topSetupRewardRisk: (metrics as Record<string, unknown>).top_setup_reward_risk ?? null,
